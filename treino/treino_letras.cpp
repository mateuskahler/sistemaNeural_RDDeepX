/**
 * Este programa demonstra treino da rede para reconhecer letras
 * 
 * para compilar:
 *
 ...
 * A rede treinada será salva no arquivo "sistema_treinado_letras", que pode ser testado com o programa
 * fornecido em "teste_letras.cpp"
 */
...
int main(int, char**)
{
	//tenta carregar amostras de banco de treino
		classes_separadas_t amostras_treino;
		{
			...
			std::cout << carrega.second << std::endl;
			if(carrega.first.empty())
			{
				std::cout << "Ocorreu um erro ao carregar banco. Abortando."<< std::endl;
				return 1;
			}
			amostras_treino = classes_separadas_t(std::move(carrega.first));
		}
		
	//exclui caracteres numéricos (10 primeiras classes)
		for(int k=0; k<10; k++)
			{ amostras_treino.exclui_classe(0);	}
	...
	//cria sistema neural
		...
		//neste exemplo, será criado um sistema de 2 camadas, com 111 e 37 neurons
		...
		//salva resultado para arquivo
			S.salva_para_arquivo(arq_sisL);
			std::cout << " resultado salvo em \'" << arq_sisL << "\' " << std::endl;
	return 0;
}